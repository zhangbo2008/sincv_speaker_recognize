...
class LayerNorm(nn.Module):
    ...
    def __init__(self, features, eps=1e-6):
        ...
        self.gamma = nn.Parameter(torch.ones(features)).to(device)
        self.beta = nn.Parameter(torch.zeros(features)).to(device)
        self.eps = eps

    def forward(self, x):
        mean = x.mean(-1, keepdim=True)
        std = x.std(-1, keepdim=True)
        return self.gamma * (x - mean) / (
                    std + self.eps) + self.beta  # 减均值除方差之后的数值已经变了,所以需要做一个放射变换再变回来, 就是乘以gamma 再 加beta. 这个也就是torch底层的实现.只不过这份代码自己手动实现了.
...
class SincNet(nn.Module):
    ...
    def forward(self, x):
        batch = x.shape[0]
        seq_len = x.shape[1]
        # 这个网络里面都用的自己实现的.没有用torch给的库包.
        if bool(self.cnn_use_laynorm_inp):
            x = self.ln0((x))

        if bool(self.cnn_use_batchnorm_inp):  # nlp任务一般不用batchnorm
            x = self.bn0((x))
